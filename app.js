const titles = [
  ...
  { id: 13, title: "Czarna kula", display: "Czarna\nkula", type: "Rekomendacja", year: "Wybór użytkownika", genre: "Tajemnice", runtime: "Ocena 7,39", userRating: "7,39", rating: "7,39 / 10", mood: "Tajemniczy", director: "Użytkownik Kadr i Czas", cast: "Tajemnice", tagline: "Historia warta drugiego spojrzenia.", synopsis: "", colors: ["#06111c", "#15314d"], posterImage: "assets/czarna-kula.png?v=20260803-3", pattern: "linear-gradient(180deg, rgba(4,12,20,.08), rgba(4,12,20,.7))" }
];
...
function artStyle(item) {
  const image = item.posterImage ? `; --poster-image: url('${item.posterImage}')` : "";
  return `--poster: linear-gradient(145deg, ${item.colors[0]}, ${item.colors[1]}); --pattern: ${item.pattern}${image}`;
}
  ...
  grid.innerHTML = items.map((item, i) => `
    <article class="title-card" style="animation-delay:${Math.min(i * 35, 220)}ms">
      <button class="poster-button" type="button" data-open="${item.id}" aria-label="Pokaż szczegóły: ${item.title}">
        <span class="poster ${item.posterImage ? "photo-poster" : ""}" style="${artStyle(item)}">
      ...
      <div class="card-meta">
        <div><h3>${item.title}</h3><p>${item.type} · ${item.userRating ? `Ocena ${item.userRating}` : item.runtime}</p></div>
...
function openDetails(id) {
  ...
  const synopsis = document.querySelector("#detailSynopsis");
  synopsis.textContent = item.synopsis;
  synopsis.hidden = !item.synopsis;
  const creditLabels = item.type === "Rekomendacja" ? ["Źródło", "Kategoria"] : ["Twórca", "Obsada"];
  document.querySelector("#detailCredits").innerHTML = `<div><small>${creditLabels[0]}</small><strong>${item.director}</strong></div><div><small>${creditLabels[1]}</small><strong>${item.cast}</strong></div>`;
  updateSavedUI();
  dialog.showModal();
}
